...
- List docker networks and explain `to-do-api_latifnet`.

```bash
docker network ls
```

- Check if the To-Do App is running by entering `curl -H "Content-Type: application/json" -X POST -d '{"title":"Get some REST", "description":"REST in Peace"}' http://52.90.136.21` in a browser.
...
- Test the application.
  ...
  ```bash
  curl curl -H "Content-Type: application/json" -X POST -d '{"title":"Get some REST", "description":"REST in Peace"}' http://52.90.136.21/todos
  ```

  - Retrieve task with `id=3` using `/todos/3` path and HTTP `GET` method with `curl` command.
  
  ```bash
  curl curl -H "Content-Type: application/json" -X POST -d '{"title":"Get some REST", "description":"REST in Peace"}' http://52.90.136.21/todos/3
  ```

  - Create new task the `To Do List` using `/todos` path and HTTP `POST` method with `curl` command.

  ```bash
  curl -H "Content-Type: application/json" -X POST -d '{"title":"Get some REST", "description":"REST in Peace"}' http://52.90.136.21//todos
  ```

  - Delete task with `id=1` using `/todos/1` path and HTTP `DELETE` method with `curl` command.
  
  ```bash
  curl -H "Content-Type: application/json" -X DELETE curl -H "Content-Type: application/json" -X DELETE -d '{"title":"Get some REST", "description":"REST in Peace"}' http://52.90.136.21/todos/1
  ```
