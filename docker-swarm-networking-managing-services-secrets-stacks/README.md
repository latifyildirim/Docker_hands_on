...
```bash
docker network create -d overlay latif-net
```

- Explain user-defined overlay network (latif-net)

```bash
docker network inspect latif-net
```
...
- Istedigimiz ec2 ya bu komutla kurariz
...
```bash
docker service create --name webserver --network latif-net -p 80:80 -d --replicas=3 clarusway/container-info:1.0
```
...
- Check the URLs of nodes that is running the task with `http://<ec2-public-hostname-of-node>` in the browser and show that the app is accessible, and explain `Container Info` on the app page. (`Host` is the name of container hosting the app, `Network Information` is giving IP addresses attached to `container` by different networks, for example;  `10.0.1.3 from latif-net`, `172.18.0.3 from docker_gwbridge`, `10.0.0.8 from ingress network`  )
...
- List the tasks and go to terminal of ec2-instance which is running `latif-db` task.

```bash
docker service ps latif-db
```

- List the containers in ec2-instance which is running `latif-db` task.

```bash
docker container ls
```

- Connect the `latif-db` container.
...
```bash
docker service rm webserver latif-db
```
...
```bash
docker stack deploy -c ./docker-compose.yml latif-todoapi
```
...
```bash
docker stack services latif-todoapi
```

- List the tasks in the stack

```bash
docker stack ps latif-todoapi
```

- Check if the `latif-todoapi` is running by entering `http://<ec2-host-name>` in a browser.

- Remove stacks.

```bash
docker stack rm latif-todoapi
```
...
```bash
docker stack deploy -c ./docker-compose.yml wplatif
```
...
```bash
docker stack services wplatif
```

- List the tasks in the stack

```bash
docker stack ps wplatif
```
